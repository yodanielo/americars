...
class MantixForm {
    ...
    function ver() {
        ...
        switch($this->atributos["mostrar_submit"]) {
            case "1": if($_POST) { $cadf.='<input name="Submit" type="button" class="form_submit" value="Actualizar '.$this->atributos["texto_submit"].'"  onClick="validar()">&nbsp;<a href="'.$_SERVER["PHP_SELF"].'" class="form_submit">Volver al listado</a>&nbaps;'; } break;
            case "0": $cadf.="";break;
            default:
                ...
                if($pre_sub=="Actualizar "){
                    ...
                    $cadf.='<input type="button" value="Volver al listado" class="form_submit" onclick="window.location.href=\''.$ruta.'\'" />';
                }
        }
        ...
        switch ($tipo) {
            ...
            case "selcadena":
                ...
                if($valor=="")
                    ...
                    $ttt=explode(",",$valor);
                //$cad='<input type="hidden" name="'.$ctl["campo"].'" value="'.$valor.'" />';
                $cad.='<div id="'.$ctl["campo"].'" class="form_selcadena">';
                for($icad=0;$icad<count($ttt);$icad++){
                    $cad.='<label><input checked onchange="quitar'.$ctl["campo"].'item(this)" type="checkbox" value="'.$ttt[$icad].'" name="scadena'.$ctl["campo"].'[]">'.$ttt[$icad].'</label><br />'."\n";
                }
                $cad.='</div><br/>';
                $cad.='<script type="text/javascript">';
                $cad.='    function agregar'.$ctl["campo"]."(){";
                $cad.='        window.open(\'files_up_'.$ctl["campo"].'.php\',\'window\', \'height=170,width=500,resizable=1\');return false;';
                $cad.='    }';
                $cad.='    function agregar'.$ctl["campo"].'item(itm){';
                $cad.='        document.getElementById("'.$ctl["campo"].'").innerHTML+=\'<label><input checked onclick="quitar'.$ctl["campo"].'item(this)" type="checkbox" value="\'+itm+\'" name="scadena'.$ctl["campo"].'[]" />\'+itm+\'</label><br/>\';';
                $cad.='    }';
                $cad.='    function  quitar'.$ctl["campo"].'item(itm){';
                $cad.='        itm.parentNode.innerHTML="";';
                $cad.='    }';
                $cad.='</script>';
                $cad.='<input type="button" class="form_submit" style="float:left; clear:both" value="Agregar fotos" onclick="agregar'.$ctl["campo"].'()" />';
                break;
            ...
            case "checkbox": $cchk="";$ant_v=0;
                if ($valor!="") {	if ($valor==1) {$cchk=" checked "; $ant_v=1;}	 }
                $cad= '<input name="'.$ctl["campo"].'" type="checkbox" class="form_chk" '.$ctl["extras"].$cchk.'" value="1"><input type="hidden" name="'.$ctl["campo"].'_ant" value="'.$valor.'" />'; break;
            case "fecha":
                if($valor==null || $valor=="")
                    $valor=date('d/m/Y');
                $cad= '<input size="10" id="' .$ctl["campo"].'" class="form_date" type="text" READONLY name="'.$ctl["campo"].'" title="DD/MM/YYYY"  value="'.$valor.'"> <input type="button" class="form_submit" value="ver calendario" onclick="scwShow(scwID(\''.$ctl["campo"].'\'),event)">'.$ast.'<input type="hidden" name="'.$ctl["campo"].'_ant value="'.$valor.'">'; break;
            ...
            case "tinymce":
                ...
                ';
                $cad .= '<textarea id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" class="form_area" style="width:530px; height:230px;"  '.$ctl["extras"].' >'.$valor.'</textarea>'.$ast; break;
            case "galeria":
            ...
            case "select":
                ...
                $cad= '<select onChange="'.$ctl["campo"].'_change(this)" id="'.$ctl["campo"].'" name="'.$ctl["campo"].'" class="form_select" '.$ctl["extras1"].'>'.$ops.'</select><input type=hidden name="'.$ctl["campo"].'_ant" value="'.$valor.'">'.$ast;
                break;
    ...
    function generar_js($label1,$campo,$valida) {
        ...
        switch ($valida) {
            case 1:  $jscad.="if (!isAlphabetic( f.".$campo.".value) ) { document.getElementById('err_".$campo."').innerHTML=\"<li>El valor del campo <strong>" .$label."</strong> debe ser únicamente letras</li>\";document.getElementById('fila_".$campo."').className='form_fila_err'; return	}"."\n";break;
            case 2:  $jscad.="if (!isNumber( f.".$campo.".value) ) { document.getElementById('err_".$campo."').innerHTML=\"<li>El valor del campo: <strong>" .$label."</strong> debe ser numérico.</li>\"; document.getElementById('fila_".$campo."').className='form_fila_err';res_valida=false;	}" ."\n";break;
            case 6:  $jscad.="if (!isDecimal( f.".$campo.".value) ) { document.getElementById('err_".$campo."').innerHTML=\"<li>El valor del campo: <strong>" .$label."</strong> debe ser numérico.</li>\"; document.getElementById('fila_".$campo."').className='form_fila_err';res_valida=false;	}" ."\n";break;
