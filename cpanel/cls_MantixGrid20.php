...
class MantixGrid {
            ...
            for($a=0; $a<count($this->buscador);$a++) {
                if($_POST['bus_'.$this->buscador[$a]["id"]]!="") {
                    ...
                    switch($tipo) {
                        ...
                        case "select" : $cad.= $this->buscador[$a]["campo"]."=".$_POST['bus_'.$this->buscador[$a]["id"]]; break;
                    }
                }
            }
        }
	...
	*/
            ...
            $r.="<tr><td height=10 ></td></tr><tr><td colspan=20 class=grid_paginado>Total de Registros: <span class=\"paginado\">".$this->datos->l_nroreg."</span> ---- ";
            if ($t_pag>1 ) {
                if ($this->p_actual >1  ) {
                    $r.='<a class="grid_nropag" href="#" onclick="grid_pagina('.($p_actual-1).')"> <<< </a>';
                }
                $r.="P&aacute;gina: <span class=\"paginado\">".$p_actual." de ".$t_pag.'</span>';
                if ($p_actual <$t_pag ) {
                    $r.='<a class="grid_nropag" href="#" onclick="grid_pagina('.($p_actual+1).')">&nbsp;>>></a>';
                }
                $r.=" &nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp; P&aacute;ginas -> &nbsp;<span class=\"paginado\">";
                for ($a=1;$a<=$t_pag;$a++) {
                    if ($a==$p_actual) {$r.="&nbsp;<span class=\"pagactual\">".$a."</span>" ;}
    ...
    function ver_js() {
        ...
        $r.="	{ "."\n";
        $r.= "    var f=document.form_grid; "."\r\n";
        $r.="	  resp=window.confirm(\"Todos los registros que dependan de éste, se borrarán.\\n¿Desea Continuar con la Eliminación del Registro?\");"."\n";
        $r.="	  if (resp)" ."\n";
        $r.="	  {"."\n";
        $r.="	    f.action ='".$url."';"."\r\n";
        $r.="	    f.accion.value=8"."\r\n";
        ...
        $r.= "      return; "."\n";
        $r.= "     }"."\n";
        $r.= '	var resp=window.confirm("Todos los registros que dependan de éste, se borrarán.\\n¿Desea Continuar con la Eliminación del Registro?");'."\n";
        $r.= "	 if (resp)"."\n";
        $r.= "	  {" ."\n";
        $r.="	    f.action ='".$url."';"."\r\n";
        $r.= "		f.accion.value=3;"."\n";
