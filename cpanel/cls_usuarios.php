...
{
 $this->ini_datos("ame_usuarios","usuario");
}
...
function ingresar()
{
	$this->datos->listaSP("select * from ame_usuarios where estado=1 and usuario='".str_replace("'","",trim($_POST["txt_user"]))."' and password='".md5(trim($_POST["txt_pass"]))."'","","");
...
if($_SESSION["user"]["password"]==trim(md5($_POST["password"]))) {
	$this->datos->ejecutar("update ame_usuarios set password='".md5(trim($_POST["nueva"]))."' where id=".$_SESSION["user"]["id"]);
...
{
$r = new MantixGrid();
$r->atributos=array("tabla"=>"ame_usuarios","nropag"=>"20","ordenar"=>"usuario");
$r->columnas=array(
